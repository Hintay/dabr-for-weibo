...
function theme_menu_toptop($menu) {
	...
	if (user_is_authenticated()){
		...
		if (setting_fetch('toppublic', 'yes') == 'yes') {
		...
		if (setting_fetch('topfriends', 'yes') == 'yes') {
			$links[] = "<a href='".BASE_URL."public'>".("关注")."</a>";
		}
...
function theme_menu_bottomtom($menu) {
	...
	if (user_is_authenticated()) {
		if (setting_fetch('replies') == 'yes') {
			$links[] = "<a href='".BASE_URL."replies'>".("提到")."</a>";
		}
	...
	return '<img src="http://img.tongji.linezing.com/1228959/tongji.gif" style="display:none;" /><div class="menu menu-$menu">'.implode(' | ', $links).'</div>';
}
...
/*function theme_menu_both($menu) {
  ...
  foreach (menu_visible_items() as $url => $page) {
    ...
    if ($menu == 'bottom' && isset($page['accesskey'])) {
      $links[] = "<a href='$url' accesskey='{$page['accesskey']}'>$title</a> ";
    } else {
  ...
  //if (user_is_authenticated()) {
    $user = user_current_username();
    array_unshift($links, "<b><a href='user/$user->screen_name'>$user->screen_name</a></b>");
  }*/
 /* if ($menu == 'bottom') {
