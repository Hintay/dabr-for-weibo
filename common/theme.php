...
function theme_page($title, $content) {
	...
	echo '<!DOCTYPE html PUBLIC "-//WAPFORUM//DTD XHTML Mobile 1.0//EN" "http://www.wapforum.org/DTD/xhtml-mobile10.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" /><link href="'.BASE_URF.'favicon.ico" rel="shortcut icon" type="image/x-icon" /><title>'.$title.$page.SINA_TITLE.'</title><base href="'.BASE_URL.'" />'.theme('css').'</head><body>'.$body.'</body></html>';
	exit();
}
...
function theme_css() {
	$c = theme('colours');
	return "<style type='text/css'>a{color:#{$c->links};}table{border-collapse:collapse;}form{margin:.3em;}td{vertical-align:top;padding:0.3em;}td a{font-weight:bold;}img{border:0;}small,small a{color:#{$c->small};font-weight:normal;}body{background:#{$c->bodybg};color:#{$c->bodyt};margin:0;font:90% sans-serif;}tr.odd td{background:#{$c->odd};}tr.even td{background:#{$c->even};}tr.reply td{background:#{$c->replyodd};}tr.reply.even td{background:#{$c->replyeven};}.menu{color:#{$c->menut};background:#{$c->menubg};padding:2px;}.menu a{color:#{$c->menua};text-decoration:none;}".setting_fetch('css')."</style>";
}
